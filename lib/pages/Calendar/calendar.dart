...
import 'package:jarvia_prime/Firebase-Interaction/Write/write.dart';
...
class JarviaCalendar extends StatefulWidget {
  const JarviaCalendar({Key? key}) : super(key: key);

  @override
  JarviaCalendarState createState() => JarviaCalendarState();
}
Map<String, dynamic> allTasksList = {};
...
String _subject = '';
String _notes = '';
String _quadrant = "Quadrant 1";
List<Task> meetingCollection = <Task>[];

class JarviaCalendarState extends State<JarviaCalendar> {
  JarviaCalendarState();
            ...
            child: FutureBuilder<List<Task>>(
              ...
              builder: (BuildContext context, AsyncSnapshot<List<Task>> snapshot){
                if(snapshot.hasData){
                  ...
                  return getEventCalendar(_calendarView, _events, onCalendarTapped);
    ...
    setState(() {
      ...
      if (_calendarView == CalendarView.month) {
        ...
        if (calendarTapDetails.appointments != null &&
          ...
          final Task meetingDetails = calendarTapDetails.appointments![0];
          _startDate = meetingDetails.from;
          _endDate = meetingDetails.to;
          _quadrant = meetingDetails.quadrant;
  ...
  Future<List<Task>> getMeetingDetails() async {
    ...
    getTasks();
    await Reader.readAllTasks().then((result) => {
      setState((){
        allTasksList = result;
      })
    });
    print("here $allTasksList");
    allTasksList.forEach((quadrant, taskList) {
      if(quadrant == "Quadrant 1"){
        meetingCollection = taskList;
      } else{
        meetingCollection.addAll(taskList);
      }
    });
...
class DataSource extends CalendarDataSource {
  ...
  @override
  MaterialColor getColor(int index) => _quadrantColorMapping[appointments![index].quadrant]!;
  ...
  String getQuadrant(int index) => appointments![index].quadrant;
}
